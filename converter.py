...
_LINK_RE = re.compile(
    "\\b(?:vless|vmess|trojan|ss|ssr|hysteria2|hy2|tuic|wireguard|wg|socks5|socks5h|socks|http|https|anytls)://[^\\s]+",
    re.IGNORECASE,
)
_TRAILING_JUNK = ".,;:!?\"'<>()[]{}\\`"
...
async def parse_batch(
    ...
    for start in range(0, len(lines), chunk_size):
        for line_no, line in enumerate(
            lines[start : start + chunk_size], start=start + 1
        ):
            if not line:
                continue
            processed += 1
            for match in _LINK_RE.finditer(line):
                ...
                try:
                    ...
                    result.skipped += 1
                    logger.warning(
                        "Строка %d: пропущена ссылка (%s): %s", line_no, scheme, exc
                    )
...
_YAML_NUMERIC_LIKE = re.compile(
    "^[-+]?(\\d[\\d_]*\\.?\\d*|\\.\\d+)([eE][-+]?\\d+)?$|^0[xX][0-9a-fA-F_]+$|^0[oO][0-7_]+$|^0[bB][01_]+$|^0[0-7]+$"
)
