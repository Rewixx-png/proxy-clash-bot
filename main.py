...
def _dedupe_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='✅ Да', callback_data='dedupe_yes'), InlineKeyboardButton(text='❌ Нет', callback_data='dedupe_no')]])
...
START_TEXT = '👋 <b>Привет! Я — конвертер прокси-ссылок в YAML для Clash Meta (Mihomo).</b>\n\n🚀 <b>Поддерживаемые протоколы:</b>\n• <b>VLESS</b> (Reality, TLS, TCP, WS, gRPC, H2, HTTPUpgrade, XHTTP)\n• <b>VMess</b> (v2rayN base64 JSON / URI)\n• <b>Trojan</b> (TLS, Reality, WS, gRPC)\n• <b>Shadowsocks (SS) &amp; SSR</b> (SIP002, Legacy, плагины obfs / v2ray-plugin)\n• <b>Hysteria 2 (hy2)</b>\n• <b>TUIC</b>\n• <b>SOCKS5 / SOCKS</b>\n• <b>HTTP / HTTPS</b>\n• <b>WireGuard (wg)</b>\n\n📤 Просто отправьте ссылки в чат (по одной на строку) или файлом <code>.txt</code> — и я сформирую <code>clash_meta_proxies.yaml</code>.\n\n👇 Подробнее — в инструкции.'
HELP_TEXT = '📖 <b>Инструкция</b>\n\n1️⃣ <b>Текст:</b> отправьте ссылки в чат, каждую с новой строки.\n2️⃣ <b>Файл:</b> прикрепите файл с расширением <code>.txt</code> — ссылки внутри будут обработаны автоматически.\n3️⃣ <b>Результат:</b> пришлю файл <code>clash_meta_proxies.yaml</code> — секцию <code>proxies</code>, которую можно вставить в ваш Clash-конфиг.\n\n⚡ <b>Поддерживаются форматы ссылок:</b>\n<code>vless://</code>, <code>vmess://</code>, <code>trojan://</code>, <code>ss://</code>, <code>ssr://</code>, <code>hy2://</code> (<code>hysteria2://</code>), <code>tuic://</code>, <code>socks5://</code>, <code>http://</code>, <code>wireguard://</code>'
NO_LINKS_TEXT = '❌ <b>Не найдено ни одной валидной прокси-ссылки.</b>\n\nПроверьте формат. Пример корректной VLESS-ссылки:\n<code>vless://UUID@server:443?type=tcp&amp;security=reality&amp;sni=example.com&amp;fp=chrome&amp;pbk=КЛЮЧ&amp;sid=ID&amp;flow=xtls-rprx-vision#Название</code>\n\nОтправьте ссылки текстом (по одной на строку) или файлом <code>.txt</code>.'
...
def _start_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='📖 Инструкция', callback_data='help')]])
...
async def handle_document(message: Message) -> None:
    ...
    if not (doc.file_name or '').lower().endswith('.txt'):
        await message.answer('⚠️ Поддерживаются только файлы <code>.txt</code>.\nПришлите текстовый файл или просто вставьте ссылки сообщением.')
        return
    try:
...
async def _progress_updater(status: Message, queue: 'asyncio.Queue[tuple[int, int, int]]') -> None:
    ...
    while True:
        ...
        if pct - last_shown >= 5 or pct >= 100:
            await status.edit_text(f'⚙️ Конвертация ссылок в YAML [{_progress_bar(pct)}] {pct}% · строк {processed}/{total}')
...
async def _process_input(message: Message, text: str) -> None:
    ...
    try:
        ...
        if dup_ips > 0:
            ...
            await status.edit_text(f'🔍 Найдено дубликатов IP-адресов: <b>{dup_ips}</b>.\n\nУбрать их из файла (оставить по одному прокси на сервер)?', reply_markup=_dedupe_keyboard())
            try:
                remove_dups = await asyncio.wait_for(fut, timeout=60)
            except asyncio.TimeoutError:
                remove_dups = False
                await status.edit_text('⏰ Не дождался ответа — оставляю дубликаты.', reply_markup=None)
            finally:
        ...
        await message.answer_document(BufferedInputFile(yaml_text.encode('utf-8'), filename='clash_meta_proxies.yaml'), caption='📄 Готовый YAML — секция <code>proxies</code> для Clash Meta')
    except Exception as exc:
