...
class Hysteria2Parser(BaseParser):
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        if scheme not in ("hysteria2", "hy2"):
            raise ValueError(
                f"ожидалась схема 'hysteria2' или 'hy2', получено '{scheme}'"
            )
        ...
        if params.get("insecure") in ("1", "true") or params.get("allowInsecure") in (
            "1",
            "true",
        ):
        ...
        if obfs:
            ...
            obfs_pwd = (
                ...
                or params.get("obfs_param")
                or params.get("obfs-param")
            )
            if obfs_pwd:
