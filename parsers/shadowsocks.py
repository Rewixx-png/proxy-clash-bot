"""Парсер Shadowsocks (ss://) ссылок в прокси-словарь Clash Meta (Mihomo).

Поддерживаются форматы:
  - SIP002: ss://BASE64(method:password)@host:port?plugin=...#Имя
  - Legacy: ss://BASE64(method:password@host:port)#Имя
  - Plain:  ss://method:password@host:port#Имя
"""
...
def _parse_plugin(plugin_str: str) -> tuple[str, dict]:
    """Разбирает плагин Shadowsocks (v2ray-plugin, simple-obfs / obfs-local, shadow-tls)."""
...
class ShadowsocksParser(BaseParser):
    """Разбирает ss:// ссылки в структуру для секции proxies."""
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        if "@" in main_part:
            # SIP002 или plain: userinfo@hostport?query
            ...
            # userinfo может быть base64(method:password) или plain method:password
            if ":" in userinfo:
        ...
        # Разбор параметров запроса (включая плагины)
        if query:
