...
_NETWORK_ALIASES = {'tcp': 'tcp', 'raw': 'tcp', 'ws': 'ws', 'websocket': 'ws', 'grpc': 'grpc', 'http': 'http', 'httpupgrade': 'http', 'h2': 'h2', 'xhttp': 'xhttp'}
...
class TrojanParser(BaseParser):
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        proxy: dict = {'name': name, 'type': 'trojan', 'server': host, 'port': int(port_s), 'password': password, 'udp': True}
