...
class TuicParser(BaseParser):
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        congestion = params.get("congestion_control") or params.get(
        ...
        if params.get("allow_insecure") in ("1", "true") or params.get("insecure") in (
            "1",
            "true",
        ):
            proxy["skip-cert-verify"] = True
        if params.get("reduce_rtt") in ("1", "true") or params.get("reduce-rtt") in (
            "1",
            "true",
        ):
            proxy["reduce-rtt"] = True
        if params.get("disable_sni") in ("1", "true") or params.get("disable-sni") in (
            "1",
            "true",
        ):
