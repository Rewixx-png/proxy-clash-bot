"""Вспомогательные функции для парсеров прокси."""
...
def split_host_port(hostport: str) -> tuple[str, str]:
    """Разделяет 'host:port', корректно обрабатывая IPv6 в виде [addr]:port."""
...
def b64decode_safe(s: str) -> bytes:
    """Безопасно декодирует строку base64 (стандартный или URL-safe, с паддингом или без)."""
...
def b64decode_str(s: str, encoding: str = "utf-8") -> str:
    """Декодирует base64 в строку UTF-8 (с игнорированием невалидных байт при необходимости)."""
    data = b64decode_safe(s)
    return data.decode(encoding, errors="replace")


def clean_name(fragment: str, fallback: str) -> str:
    """Извлекает и очищает имя прокси из URL-фрагмента (#) или использует fallback."""
    if fragment:
...
def validate_reality(pbk: str, sid: str) -> None:
    """Проверяет REALITY-параметры ровно как Mihomo (adapter/outbound/reality.go)."""
    s = pbk.strip()
    ...
    if len(raw) != 32:
        raise ValueError(
            f"невалидный REALITY public-key (ожидается 32 байта, получено {len(raw)})"
        )
