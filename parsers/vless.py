"""Парсер VLESS-ссылок в прокси-словарь Clash Meta (Mihomo).

Поддерживаемые комбинации:
  - transport: tcp (и алиас raw), ws, grpc, http (алиас httpupgrade), h2, xhttp
  - security:  none / tls / xtls / reality
  - опции:     sni, host, fp, flow, pbk, sid, path, serviceName, alpn

Формат ссылки (v2rayN):
  vless://UUID@host:port?type=...&security=...&sni=...&flow=...#Имя
"""
...
# Транспорты, которые умеет собирать Clash Meta для VLESS, и алиасы
_NETWORK_ALIASES = {
    "tcp": "tcp",
    "raw": "tcp",
    "ws": "ws",
...
# Security-режимы, дающие TLS-слой.
_TLS_SECURITIES = {"tls", "xtls", "reality"}


class VlessParser(BaseParser):
    """Разбирает vless:// ссылки в структуру для секции proxies."""
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        # Параметры запроса: тип транспорта, security, sni и т.д.
        ...
        if network is None:
            raise ValueError(
                f"транспорт '{params.get('type')}' не поддерживается Clash Meta"
            )
