...
_NETWORK_ALIASES = {'tcp': 'tcp', 'raw': 'tcp', 'ws': 'ws', 'websocket': 'ws', 'grpc': 'grpc', 'http': 'http', 'h2': 'h2', 'xhttp': 'xhttp'}
...
class VmessParser(BaseParser):
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        proxy: dict = {'name': name, 'type': 'vmess', 'server': server, 'port': port_num, 'uuid': uuid, 'alterId': alter_id, 'cipher': cipher, 'udp': True}
        ...
        if network == 'ws':
            ...
            if type_hdr == 'http':
                proxy['http-opts'] = {'path': [path or '/'], 'headers': {'Host': [host or server]}}
    ...
    def _parse_uri(cls, raw: str) -> ParsedProxy:
        ...
        proxy: dict = {'name': name, 'type': 'vmess', 'server': host, 'port': int(port_s), 'uuid': uuid, 'alterId': int(params.get('alterId', 0)), 'cipher': params.get('cipher', 'auto'), 'udp': True}
