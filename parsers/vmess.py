"""Парсер VMess-ссылок в прокси-словарь Clash Meta (Mihomo).

Поддерживаются оба формата:
  1. Base64 JSON (v2rayN): vmess://<base64-json>
  2. URI-формат: vmess://UUID@host:port?param=val...#Имя
"""
import json
...
class VmessParser(BaseParser):
    """Разбирает vmess:// ссылки в структуру для секции proxies."""
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        # Если ссылка содержит '@', пробуем разобрать её как обычный URI
        if "@" in payload:
            return cls._parse_uri(raw)

        # Стандартный формат v2rayN: base64(json)
        # Отрезаем возможный фрагмент #name после base64 если есть
        ...
        if network == "ws":
            ...
            opts: dict = {
                "path": [path or "/"] if network == "http" else path or "/"
            }
