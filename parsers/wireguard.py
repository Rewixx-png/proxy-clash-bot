...
class WireguardParser(BaseParser):
    ...
    def parse(cls, raw: str) -> ParsedProxy:
        ...
        if scheme not in ("wireguard", "wg"):
            raise ValueError(
                f"ожидалась схема 'wireguard' или 'wg', получено '{scheme}'"
            )
        ...
        private_key = (
            userinfo or params.get("private_key") or params.get("privatekey") or ""
        )
        public_key = (
            params.get("public_key")
            or params.get("publickey")
            or params.get("pubkey")
            or ""
        )
        ...
        psk = (
            params.get("preshared_key")
            or params.get("presharedkey")
            or params.get("psk")
        )
        if psk:
        ...
        if reserved:
            try:
                if "," in reserved:
                    proxy["reserved"] = [
                        int(x.strip()) for x in reserved.split(",") if x.strip()
                    ]
                elif "[" in reserved:
                    import json
